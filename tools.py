...
class MyState(object):          #action
    ...
    def distance_but_ball(self): #retourne la distance entre les buts et la balle
        ...
        if self.est_team1():
            return self.ball_position().distance(Vector2D(0,GAME_HEIGHT/2))
        else: 
            return self.ball_position().distance(Vector2D(GAME_WIDTH,GAME_HEIGHT/2))
       
    def position_cages(self,id_team): #retourne la position de self.cage
...
def joueur_plus_proche(id, state):
    mystate=MyState(self.state,self.idt,self.idp)
    pos_ball = mystate.ball_position
    def get_second(p):
        return p[1]
...
class Action(object): 
    ...
    def aller_vers_balle(self):
        mystate = MyState(self.state,self.idt,self.idp)
        return SoccerAction(Vector2D(mystate.ball_position()- mystate.my_position()),Vector2D(0,0))
    ...
    def aller_vers_but(self):
        ...
        if mystate.est_team1():
            return SoccerAction(POS_DEFAUT-mystate.my_position(),Vector2D(3.14,20))
        return SoccerAction(POS_DEFAUT2-mystate.my_position(),Vector2D(6.18,20))
    ...
    def passe(self,state,id_team,id_player): 
        ...
        if distance <= PLAYER_RADIUS + BALL_RADIUS:
            if pos.joueur_le_plus_proche(id_team,id_player) != False: 
            ...
            return SoccerAction(pos.joueur_le_plus_proche(id_team,id_player)-mystate.my_position(),shoot(self))
        
    def shoot(self,state,id_team,id_player):
        mystate = MyState(self.state,self.idt,self.idp)
        sh = Vector2D(angle=0,norm=0)
        if mystate.distance_ball_player() < PLAYER_RADIUS + BALL_RADIUS:
            if mystate.distance_but_ball() < 40: 
                sh = Vector2D(angle=3.14,norm=10)
            if mystate.distance_but_ball() > 40:# and personne(): 
                sh = Vector2D(angle=3.14,norm=20)
        return sh
